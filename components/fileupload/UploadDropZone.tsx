...
export function UploadDropZone({ propertyId, slug }) {
  ...
  async function onFilesAdded(addedFiles) {
    ...
    if (imageUrls.length > 0) {
      const result = await uploadImages(propertyId, imageUrls);
      if (result.success) {
        const statusUpdateResult = await updatePropertyStatus(
          propertyId,
          "IN_PROGRESS",
        );
        router.push(`/property/${slug}/pictures`);
        setLoading(false);
      } else {
        toast({
          title: "Error:",
          description: "Unable to upload some of your files, please try again.",
        });
        setLoading(false);
      }
    } else {
