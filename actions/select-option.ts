...
export async function selectOption(imageId, selectedOptionKey) {
  console.log(`Attempting to update selected option for imageId: ${imageId} with selectedOptionKey: ${selectedOptionKey}`);
  ...
  try {
    ...
    console.error(`Error when calling prisma.image.update for imageId ${imageId} with selectedOptionKey ${selectedOptionKey}:`, error);
